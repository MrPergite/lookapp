...
export async function DELETE(request: NextRequest) {
  try {
    // Check if the request has an authorization header
    const authHeader = request.headers.get('Authorization');
    console.log("Authorization header in delete shoppingListNew+api:", authHeader);
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      return Response.json(
        { error: 'Unauthorized: Missing or invalid token' },
        { status: 401 }
    ...
    // Parse the request body
    const body = await request.json();
    console.log("Request body in delete shoppingListNew+api:", body);
    ...
    console.error('Shopping List New API error:', error);
    return Response.json({ error: "Failed to submit product card reaction" }, { status: error.response?.status || 500 });
  }
}
